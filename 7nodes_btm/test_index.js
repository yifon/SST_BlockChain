...
var initCallback= function(jsonRes){
	...
	*/
	...
	var contractInitVars = {contractABI:jsonRes.contractABI, web3http:"http://localhost:7101", atmAdd:jsonRes.accounts[1].atms[0].address, ownerBank:jsonRes.accounts[1].address, contractAdd:jsonRes.contractAdd};
	...
	var postCreditATMPResponse = function(atmpRes, contractOutput){
		console.log("confirm credit now");
	...
	http.createServer(function (req,httpRes){
		...
		if(data.debitBank){//don't know why there is unknown call with empty data
				var contractInput ={
				_fromAtm: contractInitVars.atmAdd, //atm1
				_debitBank:data.debitBank, //bankB
				_creditBank:data.creditBank,//bankA 
				_trxHash: "{\"trxType\":\""+data.trxType
				+"\",\"abc\":12345}",
