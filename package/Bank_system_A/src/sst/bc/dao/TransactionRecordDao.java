...
public class TransactionRecordDao {
	...
	public List<TransactionRecord> getRecordByAccount(String Account) {
		...
		try {
			...
			String sql = "SELECT * FROM bank_a_transaction_record where Account = ?";
	...
	public List<TransactionRecord> getRecordByDate(String date) {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "SELECT * FROM bank_a_transaction_record where date = ?";
			System.out.println("conn="+conn);
			ps = conn.prepareStatement(sql);
			ps.setString(1, date);
			rs = ps.executeQuery();
	...
	public List<TransactionRecord> getRecordByAqbank(String aqbank) {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "SELECT * FROM bank_a_transaction_record where aqbank = ?";
			System.out.println("conn="+conn);
			ps = conn.prepareStatement(sql);
	...
	public List<TransactionRecord> getAllRecord() {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "select * from bank_a_transaction_record";
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			while (rs.next()) {
				int id = rs.getInt("id");
	...
	public boolean saveRecord(TransactionRecord transactionRecord) {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "insert into bank_a_transaction_record (txnType,Account,fromAccount,toAccount,amount,fee,date,aqbank,atmid) values(?,?,?,?,?,?,?,?,?)";
			ps = conn.prepareStatement(sql);
