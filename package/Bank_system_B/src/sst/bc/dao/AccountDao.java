...
public class AccountDao {
	...
	public CardToken getCardTokenInfo(String card_Number) {
		...
		try {
			...
			String sql = "SELECT * FROM bank_b_accounts where cardNumber = ?";
	...
	public List<CardToken> getAllCards() {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "select * from bank_b_accounts";
			ps = conn.prepareStatement(sql);
			rs = ps.executeQuery();
			while (rs.next()) {
				int id = rs.getInt("id");
	...
	public boolean acocuntAdd(CardToken cardToken) {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "insert into bank_b_accounts (cardNumber,cardHolder,balance,pin) values(?,?,?,?)";
			ps = conn.prepareStatement(sql);
	...
	public Boolean deleteCardToken(CardToken cardToken) {
		...
		try {
			conn = jdbc.getConnection();
			String sql = "delete from bank_b_accounts where id = ?";
			ps = conn.prepareStatement(sql);
	...
	public boolean updatePin(CardToken cardToken) {
		...
		try {
			...
			// pin);
			String sql = "update bank_b_accounts set pin = ? where id = ?";
			ps = conn.prepareStatement(sql);
	...
	public boolean updateBalance(CardToken cardToken) {
		...
		try {
			...
			// cardToken = new CardToken(id, cardNumber, cardHolder,balance,
			// pin);
			String sql = "update bank_b_accounts set balance = ? where id = ?";
			ps = conn.prepareStatement(sql);
